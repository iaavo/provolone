...
public class ProvoloneTestRunner extends BlockJUnit4ClassRunner {
	...
	class MethodInvoker extends Statement {
		...
		MethodInvoker(FrameworkMethod testMethod, Object target,
				FailureScreenshotTaker screenshotTaker) {
	...
	/**
	 * Returns a <code>{@link Statement}</code> that invokes {@code method} on
	 * {@code test}. The created statement will take and save the screenshot of
	 * the desktop in case of a failure.
	 */
	@Override
	protected Statement methodInvoker(FrameworkMethod method, Object test) {
		return new MethodInvoker(method, test, new FailureScreenshotTaker(
				new ImageFolderCreator().createImageFolder()));
	}
}
